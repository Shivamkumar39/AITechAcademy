...
function Login() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setErrorExits(true)
      setResponse(error?.response?.data?.error || "Login failed. Please try again.")
    } finally {
      setLoading(false)
    }
  }
  return (
    <>
      ...
      <div className='login-page' onClick={() => setErrorExits(false)}>
        <form className='container login-card' onSubmit={submit}>
          ...
          <button type="submit" className="login-btn mt-3" style={{ display: loading ? "none" : "" }}>Login</button>
